$(function() {
	...
	$.ajax({
		type: "POST",
		url: "http://www.decaihui.com/wxweb/api/vip/departments.do",
		async: false,
		contentType: "application/json",
		success: function(data) {
			for(var i = 0; i < data.data.length; i++) {
				filterDepart.push(data.data[i].department)
			}
			allDepart = data.data
	...
	for(var i = 0; i < allDepart.length; i++) {
		var lis = ""
		lis += "<li><span class='depart'>" + allDepart[i].department + "</span><span>" + allDepart[i].number + "人</span><img src='http://www.decaihui.com/wxweb/img/select.png'></li>"
	...
	if(historyRecord == null || historyRecord == "" || historyRecord == undefined) {
		...
		filterDepart = historyRecord.departments
	...
	function setListData(curPageData) {
		for(var i = 0; i < curPageData.length; i++) {
			...
			lis += "<li>"
			lis += "<div class='info'>"
			lis += "<p><span class='name'>" + curPageData[i].name + "</span> 资金账户 " + curPageData[i].clientCode + "</p>"
			lis += "<p>" + curPageData[i].mobile + "</p>"
			lis += "<p><span>" + curPageData[i].department + "</span></p>"
			lis += "</div>"
			lis += "<div class='btn jy' data-clientcode='" + curPageData[i].clientCode + "'>我的久赢</div>"
			lis += "<div class='btn report' data-clientcode='" + curPageData[i].clientCode + "'>月报历史</div>"
			lis += "</li>"
		...
		$(".jy").each(function(i) {
			$(this).click(function() {
				var tzUrl = `http://www.decaihui.com/wxweb/api/vip/myjy/${$(this).data("clientcode")}.do`
		...
		$(".report").each(function(i) {
			$(this).click(function() {
				var tzUrl = `http://www.decaihui.com/wxweb/api/vip/history/${$(this).data("clientcode")}/mine.do`
				window.location.href = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx7806a150c562b73c&redirect_uri=" + encodeURIComponent(tzUrl) + "&response_type=code&scope=snsapi_base&connect_redirect=1#wechat_redirect"
			})
		})
	}
	...
	function getListDataFromNet(pdType, pageNum, pageSize, successCallback, errorCallback) {
		...
		if(pdType < 4) {
			ajaxUrl = "http://www.decaihui.com/wxweb/api/vip/listUsers.do"
		} else {
			ajaxUrl = "http://www.decaihui.com/wxweb/api/vip/ratioListUsers.do"
		}
		//延时一秒,模拟联网
		setTimeout(function() {
			var historyObj = obj
