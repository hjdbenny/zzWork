...
function operation($scope, $http, $interval) {
	...
	$.ajax({
		type: "POST",
		url: "http://www.decaihui.com/wxweb/api/month/typeDetails.do",
		async: true,
		...
		success: function(data) {
			if(data.data.length == 0) {
				...
				if(sessionStorage.getItem("isVIP") == "0") {
					window.location.href = `http://www.decaihui.com/wxweb/api/view/myMonthlyReport/${$scope.openID}/${effectiveTime}/${$scope.date}.do`
				} else {
					window.location.href = `http://www.decaihui.com/wxweb/api/vip/one/${$scope.clientCode}/${$scope.openID}/${effectiveTime}/${$scope.date}/mine.do`
				}
			}
			loadData(data.data)
		}
