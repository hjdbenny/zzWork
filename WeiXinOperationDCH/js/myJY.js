...
function operation($scope, $http, $interval, $timeout) {
	...
	$.ajax({
		type: "POST",
		url: "http://www.decaihui.com/wxweb/api/total/updateTime.do",
		async: false,
	...
	$.ajax({
		type: "POST",
		url: "http://www.decaihui.com/wxweb/api/total/endDatePersonalInfo.do",
		async: false,
		data: JSON.stringify(obj),
		contentType: "application/json",
		xhrFields: {
	...
	function getRank(type) {
		...
		$.ajax({
			type: "POST",
			url: "http://www.decaihui.com/wxweb/api/total/rankByType.do",
			async: true,
			contentType: "application/json",
			data: JSON.stringify(obj),
			xhrFields: {
