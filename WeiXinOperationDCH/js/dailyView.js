$(function() {
	...
	$.ajax({
		type: "POST",
		url: `http://www.decaihui.com/wxweb/api/dailyView/listReleasedDisplayDailyViews/${cxDate}.do`,
	...
	function loadCardData(arr) {
		...
		for(var i = 0; i < arr.length; i++) {
			...
			if(newContent.positionCondition == 0) {
				imgSrc = "http://www.decaihui.com/wxweb/img/more.png"
			} else if(newContent.positionCondition == 1) {
				imgSrc = "http://www.decaihui.com/wxweb/img/empty.png"
			} else if(newContent.positionCondition == 2) {
				imgSrc = "http://www.decaihui.com/wxweb/img/flat.png"
			}
			var lis = ""
			...
			lis += "<img src='http://www.decaihui.com/wxweb/img/rotate.png' class='analyst_level_bg' />"
			lis += "<img src='http://www.decaihui.com/wxweb/img/expert.png' class='analyst_level' />"
			lis += "</div>"
			lis += "</div>"
	...
	function getTips(strDate) {
		$.ajax({
			type: "POST",
			url: "http://www.decaihui.com/wxweb/api/dailyView/getDailyViewGuide/" + strDate + ".do",
			contentType: "application/json",
			async: false,
			xhrFields: {
				withCredentials: true
	...
	function tzHistory() {
		$(".swiper-wrapper>li").each(function() {
			$(this).click(function() {
				...
				window.location.href = "http://www.decaihui.com/wxweb/dailyView_detials.html"
			})
		})
	}
	tzHistory()
	...
	function cxPublishedDate() {
		...
		$.ajax({
			type: "POST",
			url: "http://www.decaihui.com/wxweb/api/dailyView/listReleasedDisplayViewDates/" + strDate + ".do",
			contentType: "application/json",
			async: false,
			xhrFields: {
				withCredentials: true
