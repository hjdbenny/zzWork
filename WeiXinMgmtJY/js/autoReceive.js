$(function() {
	...
	function loaded() {
		$.ajax({
			type: "POST",
			url: "http://wx.zzfco.com/wxmgmt/api/wxapi/listReplyMessages/0.do",
			async: true,
	...
	function loadReview(arr) {
				...
				if(jsonObj2.xml.MsgType == "news") {
					...
					if(jsonObj2.xml.ArticleCount > 1) {
						...
						for(var j = 0; j < jsonObj2.xml.ArticleCount; j++) {
							...
							articlePicUrl.push(jsonObj2.xml.Articles.item[j].PicUrl)
						}
						...
						nlis += "</div>"
						$(".reply_container").append($(nlis))
						articlePicUrl.push(jsonObj2.xml.Articles.item.PicUrl)
						var navli = "<li class='style_news newsMore'><img src='img/news.png'></li>"
						$(navli).insertBefore($(".addNews"))
					}
					$(".newsAll").eq(0).show()
					...
					$(".reply_pic img").attr("src", "http://wx.zzfco.com/wxservice/wxapi/getMediaFile/" + jsonObj2.xml.Image.MediaId + ".do")
		...
		$(".del").each(function() {
			$(this).click(function() {
				...
				$.ajax({
					type: "POST",
					url: "http://wx.zzfco.com/wxmgmt/api/wxapi/deleteReplyMessages.do",
					async: true,
					contentType: "application/json",
					xhrFields: {
						withCredentials: true
...
//多个图文消息添加
	...
	$(".post_pic").change(function() {
		...
		$.ajax({
			url: "http://wx.zzfco.com/wxservice/wxapi/uploadMediaFile.do",
			type: 'POST',
			cache: false,
	...
	function uploadArticPic(){
		$(".article_pic").each(function(i) {
			$(this).change(function() {
				...
				$.ajax({
					url: "http://wx.zzfco.com/wxservice/wxapi/uploadMassSendImage.do",
					type: 'POST',
					cache: false,
					...
					success: function(data) {
						console.log(data)
						articlePicUrl.push(data.data.url)
					}
				})
			})
		})
	...
	function newAddConfirm(obj, replyTpye) { //replyTpye消息回复类型，1为消息自动回复，0为关键字回复
		$.ajax({
			type: "POST",
			url: "http://wx.zzfco.com/wxmgmt/api/wxapi/saveOrUpdateReplyMessage.do",
			async: true,
			contentType: "application/json",
			xhrFields: {
				withCredentials: true
	...
	function loadAutoReply() {
		$.ajax({
			type: "POST",
			url: "http://wx.zzfco.com/wxmgmt/api/wxapi/getAutoReplyMessage.do",
			async: true,
			contentType: "application/json",
			xhrFields: {
				withCredentials: true
			...
			success: function(data) {
				...
				if(jsonObj3.xml.MsgType == "text") {
					...
					$(".auto_pic").show()
					$(".auto_text_content").val("")
					$(".auto_pic img").attr("src", "http://wx.zzfco.com/wxservice/wxapi/getMediaFile/" + jsonObj3.xml.Image.MediaId + ".do")
				}
			}
		});
	}
