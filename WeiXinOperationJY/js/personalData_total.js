...
function operation($scope, $http, $interval) {
	...
	$.ajax({
		type: "POST",
		url: "http://wx.zzfco.com/wxweb/api/month/personalInfo.do",
		async: true,
	...
	$(".more").click(function(e) {
		e.preventDefault()
		window.location.href = `http://wx.zzfco.com/wxweb/personalData_more.html?timestamp=${new Date().getTime()}&isChild=true`
	})
	//曲线图
	$.ajax({
		type: "POST",
	...
	$.ajax({
		type: "POST",
		url: "http://wx.zzfco.com/wxweb/api/personal/getMyMonthlyReport.do",
		async: false,
		contentType: "application/json",
	...
	$.ajax({
		type: "POST",
		url: "http://wx.zzfco.com/wxweb/api/month/topThree/" + cxMonth + ".do",
		async: true,
		contentType: "application/json",
		success: function(data) {
			for(var i = 0; i < data.data.length; i++) {
				var lis = ""
				lis += "<li>"
				lis += "<div class='liLeft'><img src='http://wx.zzfco.com/wxweb/img/" + (i + 1) + "ST.png'/></div>"
