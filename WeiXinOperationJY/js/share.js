$(function() {
	...
	function create_signature() {
		...
		$.ajax({
			type: "POST",
			url: "http://wx.zzfco.com/wxservice/wxapi/sign.do",
			async: false,
	...
	wx.ready(function() {
		...
		if(shareHasGift){
			...
			var shareLink = `http://wx.zzfco.com/wxweb/shareTransition.html?timestamp=${new Date().getTime()}&before=${getUrlParam("before")}`
		}else{
			var shareUrl = window.location.href.slice(33)
			var shareLink = `http://wx.zzfco.com/wxweb/shareTransition.html?timestamp=${new Date().getTime()}&shareUrl=${shareUrl}`
		}	
		var shareData = {
			title: title,
			desc: desc,
			...
			imgUrl: "http://wx.zzfco.com/wxweb/img/shareLogo.jpg",
			success: function(res) {
				var shareObj = {
					url: window.location.href.split('?')[0]
				}
				$.ajax({
					type: "POST",
					url: "http://wx.zzfco.com/wxweb/api/monthlyReport/addShareTimes.do",
					async: true,
