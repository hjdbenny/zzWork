...
function operation($scope, $http, $interval) {
	...
	$.ajax({
		type: "POST",
		url: "http://wx.zzfco.com/wxweb/api/total/endDateTypeDetails.do",
		async: true,
		...
		success: function(data) {
			if(data.data.length == 0) {
				...
				if(sessionStorage.getItem("isVIP")=="0"){
					window.location.href = "http://wx.zzfco.com/wxweb/api/view/myjy/" + $scope.openID + "/" + effectiveTime + ".do"	
				}else{
					var tzUrl = `http://wx.zzfco.com/wxweb/api/vip/myjy/${$scope.clientCode}.do`
	...
	function loadData(arr) {
		...
		$(".hy").each(function(i) {
			$(this).click(function() {
				...
				window.location.href = `http://wx.zzfco.com/wxweb/myJY_details.html?timestamp=${new Date().getTime()}&isChild=true`
			})
		})
	}
	$.footerSkip()
