...
//	/设置选择的年份
	...
	$.ajax({
		type: "POST",
		url: "http://www.decaihui.com/wxmgmt/api/myMonthlyReport/listAllTemplates.do",
		async: false,
	...
	$(".toDel").each(function() {
		$(this).click(function() {
			$.ajax({
				type: "post",
				url: `http://www.decaihui.com/wxmgmt/api/myMonthlyReport/deleteTemplate/${$(this).parent().parent().parent().parent().data("id")}.do`,
				async: true,
				contentType: "application/json",
				success: function(data) {
					if(data.code == "000") {
...
//		$(".description").html("")
	...
	$("#saveModule").click(function(e) {
		...
		if(confirm(`确认保存${$(".description").text()}?`)){
			$.ajax({
				type: "post",
				url: "http://www.decaihui.com/wxmgmt/api/myMonthlyReport/saveOrUpdateTemplate.do",
				async: true,
				contentType: "application/json",
